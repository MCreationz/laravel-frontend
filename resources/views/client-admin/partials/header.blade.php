...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadNotifications() {
        ...
        try {
            ...
            const response = await fetch(
                "{{ route('client-admin.notifications.index') }}"
            );
    ...
    window.Echo
        .private('client-admin.{{ auth("client_admin")->id() }}')
    ...
    document.addEventListener('click', async function (e) {
        ...
        await fetch(`/client-admin/notifications/${id}/read`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN':
    ...
    document.getElementById('mark-all-read')
        ?.addEventListener('click', async function () {
            ...
            await fetch(
                "{{ route('client-admin.notifications.read-all') }}",
                {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN':
