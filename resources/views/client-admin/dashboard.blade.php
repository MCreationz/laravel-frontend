...
$activeReviewers = \App\Models\Reviewer::where('client_id', $clientId)
...
$openFunds = (clone $fundsQuery)
    ->where('status', 'active')
    ->count();

$activeFunds = (clone $fundsQuery)
    ->where('status', 'active')
    ->count();

$closedFunds = (clone $fundsQuery)
    ->where('status', 'closed')
    ->count();

$suspendedFunds = (clone $fundsQuery)
    ->where('status', 'suspended')
    ->count();

$draftFunds = (clone $fundsQuery)
    ->where('status', 'draft')
    ->count();

$totalStatusFunds = max(
    1,
    $activeFunds + $closedFunds + $suspendedFunds + $draftFunds
);
...
$recentApplications = \App\Models\FundApplication::with([
        'fund',
        'organization'
    ])
